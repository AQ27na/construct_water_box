...
def main():
    ...
    parser.add_argument("--d", type=float, default=1.0, help="density in g/cm^3 (default 1.0)")
    parser.add_argument("--water", type=str, default="spce.pdb", help="Path to a single water molecule pdb file")
    ...
    n, box = compute_n(args.x, args.y, args.z, args.d)
    water_prefix = Path(args.water).stem
    args.out = f"{water_prefix}_{n}.pdb"
    Lx, Ly, Lz = box
